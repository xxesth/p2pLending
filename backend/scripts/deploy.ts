...
async function main() {
  ...
  const oracle = await MockOracle.deploy(8, 200000000000); // $2000
  ...
  console.log("✅ LendingToken deployed to:", tokenAddress);
  ...
  console.log("✅ LendingPlatform deployed to:", platformAddress);
  ...
  console.log("💧 Seeded deployer with 1000 DUSD");

  // --- AUTOMATIC FRONTEND CONFIGURATION ---
  console.log("📝 Writing to Frontend constants...");
...
export const PLATFORM_ABI = ${JSON.stringify(PlatformArtifact.abi, null, 2)} as const;
...
`;
  ...
  console.log(`✨ Config written to: ${frontendPath}`);
}
